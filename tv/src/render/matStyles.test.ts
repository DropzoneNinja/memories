...
test('inner bevel keeps the original raised look: outer shadow layers plus a bright hairline highlight', () => {
  ...
  assert.ok(!css.includes('rgba(0,0,0,0.4)'), 'raised look must not include the recessed dark inset band');
});

test('recessed bevel drops the outer shadow and uses an inset shadow instead', () => {
